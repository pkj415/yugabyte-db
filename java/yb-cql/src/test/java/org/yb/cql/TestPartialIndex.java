...
public class TestPartialIndex extends BaseCQLTest {
  ...
  protected Map<String, String> getTServerFlags() {
    ...
    flagMap.put("cql_raise_index_where_clause_error", "false");
  ...
  private void assertAlreadyInsertedTrueRowsPresence(List<String> projCols,
    ...
    for (int i = 0; i < alreadyInsertedTrueRows.size(); i++) {
      ...
      assertTrue(idxTuples.contains("Row[" + row + "]"));
    }
    assertTrue(idxTuples.size() == alreadyInsertedTrueRows.size());
  }
  ...
  public void testPartialIndexSelectInternal(String whereClause, List<Object> bindValues,
    ...
    while (true) {
      ...
      GetTableSchemaResponse response = miniCluster.getClient().getTableSchema(DEFAULT_TEST_KEYSPACE, testTableName);
      List<IndexInfo> indexes = response.getIndexes();

      for (IndexInfo index : indexes) {
        if (index.getIndexPermissions() != Common.IndexPermissions.INDEX_PERM_READ_WRITE_AND_DELETE) {
