		...
		if (is_ddl)
		{
			...
			else
			{
				...
				if (!(yb_enable_ddl_savepoint_infra &&
					...
					YBTransactionContainsNonReadCommittedNamedSavepoint())
					ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
...
YbCanSkipIntents(Relation rel, bool is_write)
{
	...
	/*
	 * Savepoint requires the use of intents db: on ROLLBACK TO SAVEPOINT sp,
	 * all intents in the current transaction that have a SubTransactionId >= sp
	 * will be marked as aborted.
	 * Consider this example:
	 * BEGIN; -- currentSubTransactionId = 1
	 ...
	 *             (SELECT * FROM generate_series(1,1000000) AS id) AS id;
	 *   SAVEPOINT sp1; -- currentSubTransactionId = 2
	 *   INSERT INTO users2 SELECT md5(random()::text), md5(random()::text) FROM
	 *             (SELECT * FROM generate_series(1,1000000) AS id) AS id;
	 *   SAVEPOINT sp2; -- currentSubTransactionId = 3
	 *   INSERT INTO users3 SELECT md5(random()::text), md5(random()::text) FROM
	 *             (SELECT * FROM generate_series(1,1000000) AS id) AS id;
	 *   ROLLBACK TO SAVEPOINT sp2; -- currentSubTransactionId = 2
	 * COMMIT;
	 * The "INSERT INTO users" can be directly written into regulardb. All the rest
	 * must go through intents db otherwise "ROLLBACK TO SAVEPOINT" will not
	 * work.
	 *
	 * If we have ever created a savepoint then currentSubTransactionId will be
	 * greater than TopSubTransactionId. But if we only have internally generated
	 * savepoint for read committed, we can skip intents write.
	 */
	if (GetCurrentSubTransactionId() > TopSubTransactionId &&
		YBTransactionContainsNonReadCommittedSavepoint())
	{
		elog(DEBUG1, "Disable skip intents due to relation %u write", rel->rd_id);
		skip_intents_txn_state.disabled = true;
		return false;
	}
	...
	elog(DEBUG1, "Skipping intents db %s for relation %u",
...
void
YbMaybeDisableSkipIntentsForXRepl(Oid database_oid)
{
	if (!yb_enable_new_relation_fastpath_write)
		return;
	...
	YbcStatus	status = YBCIsNamespacePartOfXRepl(database_oid,
												   &is_namespace_part_of_xrepl);
	if (status)
	{
		/*
		 * During rolling upgrades an old master may not support this RPC yet.
		 * Conservatively treat failure as "part of xRepl" to avoid skipping
		 * intents for tables that might be part of CDC replication.
		 */
		elog(DEBUG1, "YBCIsNamespacePartOfXRepl failed for database %u, "
			 "conservatively disabling skip intents", database_oid);
		YBCFreeStatus(status);
		skip_intents_txn_state.disabled = true;
		return;
	}
