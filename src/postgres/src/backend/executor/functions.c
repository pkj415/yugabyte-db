					 ...
					 bool lazyEvalOK)
{
	 ...
	 *
	 * TODO(read committed): Support lazy evaluation (#12959)
	 */
																		...
																		int yb_rc_retry_attempt_num)
{
	 ...
	 * one. Hence the "!*pushed_snapshot" assertion below.
	 *
	 * Also, a retry is expected only in Read Committed isolation level.
	 *
	 * Also, a retry is not expected to occur in the following situations (reasons
	 * explained in detail in fmgr_sql:
	 *  (i) Lazy functions (!fcache->lazyEval)
	 *  (ii) Read only functions (!fcache->readonly_func)
	 *  (iii) DDL statements (YBGetDdlNestingLevel() == 0)
	 */
	...
	while (*es)
	{
		...
		/*
		 * Advance to next execution_state.
		 */
		*es = (*es)->next;
	}
}
																									...
																									bool *pushed_snapshot)
{
	...
	{
		char *msg = psprintf("[SQL func/proc]: Can't retry code %d", edata->yb_txn_errcode);
	...
	if (!IsYBReadCommitted())
	{
		yb_read_committed_retry_log_and_throw_error(
				"[SQL func/proc]: Use READ COMMITTED isolation to retry serialization errors",
				edata, error_context);
	}
	...
	*pushed_snapshot = false;
	...
	{
		/*
		 * TODO(read committed): ensure we don't reset the read manipulation state
		 * once we set this.
		 */
...
fmgr_sql(PG_FUNCTION_ARGS)
{
	...
	bool		is_first;
	bool		pushed_snapshot;
	bool starts_at_query_boundary; /* YB: only used in an assertion */
	execution_state *es;
	TupleTableSlot *slot;
	Datum		result;
	List	   *eslist;
	...
	while (es)
	{
		...
		PG_TRY();
		{
			...
			SubTransactionId yb_rc_internal_sub_txn_id = InvalidSubTransactionId;
					...
					YBGetDdlNestingLevel() == 0)
			{
				 ...
				 *	 already returned some rows to the caller, and the current execution
				 ...
				 *   each statement is supposed to use the same snapshot till it
				 *	 completes.
				 *
				 * (2) !fcache->readonly_func:
				 *   Read only functions use the snapshot of the surrounding query.
				 *   Restarting such a query would require using a new snapshot so that
				 *	 the error doesn't occur again. But oeprating on a different
				 *	 snapshot would break the guarantee that the read only func and
				 *	 surrounding query use the same snapshot.
				 *
				...
				yb_rc_internal_sub_txn_id = GetCurrentSubTransactionId();
			}
			...
			{
				/*
				 * We need to flush buffered ops so that we retry for any errors that might occur in
				 * the not-yet-flushed ops. This is already done in ReleaseCurrentSubTransaction().
				 *
				 * Note that we expect that the internal sub-txn started for READ COMMITTED isolation
				 * level is the current ... one still i.e., no sub-txns have been added or removed.
				 * This is okay to expect since SQL functions and procedures can't have statements that
				 * change the transactional state.
				 */
				Assert(yb_rc_internal_sub_txn_id == GetCurrentSubTransactionId());
				ReleaseCurrentSubTransaction();
			}

			elog(LOG, "Piyush - es == NULL %d", es == NULL);
		...
		PG_CATCH();
		{
			...
			yb_sql_func_statement_attempt_to_restart_on_error(
					yb_rc_retry_attempt_num, old_context, fcache, es, &pushed_snapshot);
