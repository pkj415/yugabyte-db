...
CreateExecutorState(void)
{
	...
	estate->yb_es_in_txn_limit_for_reads_already_set = false;
	...
	/*
	 * Pointer to a bool which helps identify if the if the in txn limit for read operations is
	 * already picked for the current query. This pointer is passed down to all the DocDB read
	 * operations invoked for this query. Only the first read operation initializes its value, and all
	 * the other operations ensure that they don't read any value written with a timestamp after the
	 * in_txn_limit that was picked.
	 */
	estate->yb_exec_params.stmt_in_txn_limit_for_reads_already_set =
		&estate->yb_es_in_txn_limit_for_reads_already_set;
