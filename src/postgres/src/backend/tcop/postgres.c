						 ...
						 bool* retries_exhausted)
{
	...
	bool is_conflict_or_error = YBCIsTxnConflictError(edata->yb_txn_errcode);
	if (!is_read_restart_error && !is_conflict_or_error)
	{
		if (yb_debug_log_internal_restarts)
	...
	if (is_conflict_or_error && attempt >= YBCGetMaxWriteRestartAttempts())
	{
		if (yb_debug_log_internal_restarts)
                      ...
                      const void* functor_context)
{
	...
	{
	  Assert(YBNumBufferedOperations() == 0);
		YBSaveOutputBufferPosition(!yb_is_begin_transaction(restart_data->command_tag));
		PG_TRY();
		{
			elog(DEBUG2, "Trying attempt no: %d for %s", attempt, restart_data->query_string);
