...
class YBTransaction::Impl final : public internal::TxnBatcherIf {
 public:
  ...
  bool Prepare(internal::InFlightOpsGroupsWithMetadata* ops_info,
    ...
    {
      yb::SharedLock<std::shared_mutex> lock(mutex_);
      ops_info->metadata = {
  ...
  void SetActiveSubTransaction(SubTransactionId id) EXCLUDES(mutex_) {
    UNIQUE_LOCK(lock, mutex_);
  ...
  std::future<Status> SendHeartBeatOnRollback(
      ...
      const AbortedSubTransactionSet& aborted_sub_txn_set) REQUIRES(mutex_) {
    ...
    manager_->rpcs().RegisterAndStart(
        PrepareHeartbeatRPC(
            ...
            [this, handle, promise](const auto& status, const auto& req, const auto& resp) {
  ...
  Status RollbackToSubTransaction(SubTransactionId id, CoarseTimePoint deadline) EXCLUDES(mutex_) {
    ...
    vector<std::future<Status>> heartbeat_futures;
    {
      ...
      if (!ready_) {
        ...
        // tablet synchronously as part of a rollback in both situations with reasoning as follows
        // for each situation -
        //
      ...
      // We are making a copy of the aborted sub txn set so that we can update it with the new range
      // and send that in heartbeats before updating the actual aborted sub txn set.
      AbortedSubTransactionSet aborted_sub_txn_set = subtransaction_.get().aborted;
      RETURN_NOT_OK(aborted_sub_txn_set.SetRange(id, subtransaction_.GetHighestSubtransactionId()));
      ...
      if (old_status_tablet_state_.load(std::memory_order_acquire) ==
        ...
        heartbeat_futures.push_back(SendHeartBeatOnRollback(
            deadline, old_status_tablet_, &old_rollback_heartbeat_handle_, aborted_sub_txn_set));
      }
      ...
      if (state == TransactionState::kRunning) {
        ...
        heartbeat_futures.push_back(SendHeartBeatOnRollback(
            deadline, status_tablet_, &rollback_heartbeat_handle_, aborted_sub_txn_set));
      }
    }
 ...
 private:
  ...
  YBSubTransaction subtransaction_ GUARDED_BY(mutex_);
