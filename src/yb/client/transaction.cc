...
struct LogPrefixTag {
  explicit LogPrefixTag(const std::string* name_ = nullptr, uint64_t value_ = 0) noexcept
...
struct TaggedLogPrefix {
  std::string prefix;
  std::atomic<LogPrefixTag> tag;
};

std::ostream& operator<<(std::ostream& str, const TaggedLogPrefix& value) {
  const auto tag = value.tag.load(std::memory_order_acquire);
  str << value.prefix;
  if (tag.name) {
...
class YBTransaction::Impl final : public internal::TxnBatcherIf {
 public:
  ...
  void SetLogPrefixTag(const LogPrefixName& name, uint64_t id) {
    log_prefix_.tag.store(LogPrefixTag(&name.Get(), id), std::memory_order_release);
