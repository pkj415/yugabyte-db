 ...
 public:
  ...
  Status Apply(const PgTableDesc& table,
    ...
    if (!Empty(buffer)) {
      ...
      if ((IsTransactional() && in_txn_limit_for_reads_num && *in_txn_limit_for_reads_num)
          || IsCatalog()) {
    ...
    return pg_session_.pg_txn_manager_->CalculateIsolation(
        read_only, txn_priority_requirement, in_txn_limit_for_reads_num);
  }
...
Result<PerformFuture> PgSession::DoRunAsync(
    const Generator& generator, uint64_t* in_txn_limit_for_reads_num,
  ...
  for (; !table_op.IsEmpty(); table_op = generator()) {
    ...
    RETURN_NOT_OK(runner.Apply(*table, *op, in_txn_limit_for_reads_num, force_non_bufferable));
  }
...
Result<PerformFuture> PgSession::RunAsync(const OperationGenerator& generator,
                                          ...
                                          ForceNonBufferable force_non_bufferable) {
  return DoRunAsync(
      generator, in_txn_limit_for_reads_num, force_non_bufferable, std::string() /* cache_key */);
}
...
Result<PerformFuture> PgSession::RunAsyncCacheable(
    const ReadOperationGenerator& generator, uint64_t* in_txn_limit_for_reads_num,
  ...
  return DoRunAsync(
      generator, in_txn_limit_for_reads_num, ForceNonBufferable::kFalse, std::move(cache_key));
}
