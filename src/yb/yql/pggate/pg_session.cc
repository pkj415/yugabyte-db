...
// All components in read times except in_txn_limit and serial_no must be equal.
...
Result<const ReadHybridTime&> ActualReadTime(
    const ReadHybridTime& read_time1, const ReadHybridTime& read_time2) {
  if (read_time1 == read_time2) {
    return read_time1;
  }
  const auto* read_time_with_in_txn_limit_max = &read_time1;
  const auto* read_time = &read_time2;
  if (read_time1.in_txn_limit != HybridTime::kMax) {
  ...
  tmp_read_time.serial_no = read_time_with_in_txn_limit_max->serial_no;
  SCHECK(
