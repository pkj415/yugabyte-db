 ...
 public:
  Result<PgDocResponse> Send(
    ...
    if (!collecting_mode_) {
      ...
      return PgDocResponse(std::move(future), in_txn_limit_for_reads_num);
    }
    ...
    RSTATUS_DCHECK(!in_txn_limit_for_reads_num, IllegalState, "Only zero is expected");
    for (auto end = ops + ops_count; ops != end; ++ops) {
      ops_.emplace_back(*ops, table);
    }
    if (!provider_state_) {
      provider_state_ = std::make_shared<ResponseProvider::State>(
          rpc::CallResponsePtr(), 0 /* in_txn_limit_for_reads_num */);
    }
 ...
 private:
  Status DoTransmitCollected(PgSession* session) {
    ...
    auto perform_future = VERIFY_RESULT(session->RunAsync(make_lw_function(
        ...
        }), &provider_state_->in_txn_limit_for_reads_num));
...
Status FetchExistingYbctids(PgSession::ScopedRefPtr session,
  ...
  auto request_sender = [&precast_sender](
    ...
    return precast_sender.Send(
        session, ops, ops_count, table, in_txn_limit_for_reads_num, force_non_bufferable);
  };
