...
information to acheive (1) & (2). If in_txn_limit_ht is set, any operation will not be able to read
provisional writes from the same transaction which have a write timestamp higher than the
in_txn_limit_ht.

YSQL acheives requirements (1) & (2) as follows:

1) for each new SQL statement, the current hybrid time is picked as the in_txn_limit which is
used for all reads that will be performed as part of that statement. Also, any write from a previous
SQL statement that a read is supposed to see, should have completed before the in_txn_limit is
picked; so that the write timestamp of that write is lower than the in_txn_limit of the read.
(See PgSession::RunHelper::Apply() where this is ensured)
...
modification of earlier writes:
   ...
   ii) Since, every batch of writes pick the current hybrid time as the in_txn_limit, if 2 batch
   of writes touch the same row, the later batch will pick an in_txn_limit only after the earlier
   batch is done. This ensures that the write timestamp of any write in the former batch is lower
   than the in_txn_limit picked by the latter batch.
...
A flush can either if the buffer is full or if later execution either depends on the result of the
writes/ requires them to complete before making progress. The following scenarios should be handled:
  ...
  3) the query engine explicitly flushes. This could happen because:
      ...
      b) beginning or releasing a sub-transaction i.e., save point (see 373c476d)
      ...
      g) When the outer most standard_ExecutorFinish is done (i.e., not a nested executor). This
         usually happens when the topmost i.e., outer-level SQL statement is down executing. For
      ...
      h) d) COPY command (TODO: explain this)
