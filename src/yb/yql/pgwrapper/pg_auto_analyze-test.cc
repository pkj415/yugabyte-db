...
class PgAutoAnalyzeTest : public PgMiniTestBase {
 protected:
  void SetUp() override {
    ...
    FLAGS_ysql_node_level_mutation_reporting_interval_ms = 50;
    FLAGS_ysql_cluster_level_mutation_persist_interval_ms = 50;

    // TODO(auto-analyze): Do PgMiniTestBase::SetUp() first once we stop using
    // ysql_enable_table_mutation_counter in TabletServer::Init().
    PgMiniTestBase::SetUp();
    FLAGS_ysql_enable_auto_analyze_service = true;

    ASSERT_OK(SET_FLAG(vmodule, "pg_client_session=4,pg_table_mutation_count_sender=5,pg_mutation_counter=5,pg_auto_analyze*=5,stateful_service*=5,catalog_manager=5"));
  ...
  void ExecuteStmtAndCheckMutationCounts(
    ...
    auto wait_for_mutation_reporting_and_persisting_ms =
        ...
        FLAGS_ysql_cluster_level_mutation_persist_interval_ms + 10;
    ...
    for (const auto& [table_id, expected_mutations]: expected_table_mutations) {
      ASSERT_EQ(
          table_mutations_in_cql_table_after[table_id] - table_mutations_in_cql_table_before[table_id],
          expected_mutations);
    }
  }
};
...
TEST_F(PgAutoAnalyzeTest, YB_DISABLE_TEST_IN_TSAN(CheckTableMutationsCount)) {
  ...
  // TODO(auto-analyze): Test the following scenarios:
  // 1. Pg connections to more than 1 node that also touch tables on more than one nodes.
  ...
  // 5. Ensure graceful shutdown of the auto analyze stateful service and the node level mutation
  //    count sender.
}
