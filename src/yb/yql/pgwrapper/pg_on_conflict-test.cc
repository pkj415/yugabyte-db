...
class InconsistentFastPathUpdateTest : public PgOnConflictTest {
 protected:
  void UpdateMiniClusterOptions(ExternalMiniClusterOptions* opts) override {
    PgOnConflictTest::UpdateMiniClusterOptions(opts);
    opts->extra_tserver_flags.push_back("--TEST_random_delay_update_consensus_max_ms=200");
  }
};
...
// Case: INSERT --> UPDATE
TEST_F(InconsistentFastPathUpdateTest, InconsistentFastPathUpdate) {
