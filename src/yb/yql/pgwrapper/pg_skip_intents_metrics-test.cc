...
TEST_F(SkipIntentsSameTxnCreatedReadGuardTest, SelectBetweenInsertsStillSkips) {
  ...
  ASSERT_GE(m2, m1) << "Plain SELECT should not reduce skip-intents write metric";
...
TEST_F(SkipIntentsSameTxnCreatedReadGuardTest, ModifyingCteDisablesFollowingInsert) {
  ...
  ASSERT_EQ(m_after_tail_insert, m_after_mcte_select)
      << "INSERT after a SELECT that has a modifying CTE (and reads a same-txn "
         "created table) should not use skip-intents";
...
TEST_F(SkipIntentsPITRTest, TestPITRAfterSkipIntentsCTAS) {
  ...
  ASSERT_OK(conn.Execute("CREATE TABLE source_data AS SELECT generate_series(1, 500) AS id"));
  ...
  // Capture the restore time BEFORE the CTAS.
  ...
  // Perform a CTAS that triggers skip-intents writes.
  auto writes_before = ASSERT_RESULT(GetSkipIntentsCount());
  ASSERT_OK(conn.Execute("CREATE TABLE pitr_test AS SELECT * FROM source_data"));
  auto writes_after = ASSERT_RESULT(GetSkipIntentsCount());

  LOG(INFO) << "Skip-intents writes during CTAS: " << writes_before << " -> " << writes_after;
  ASSERT_GT(writes_after, writes_before)
      << "CTAS should have used skip-intents (data written directly to regular DB)";
  ...
  // Restore to the time before the CTAS.
  ...
  // After restore, the CTAS-created table should not exist in PG catalog.
  ...
  // The source table should still exist.
  auto source_count = ASSERT_RESULT(
      conn.FetchRow<PGUint64>("SELECT count(*) FROM source_data"));
...
TEST_F(SkipIntentsPITRTest, TestPITRAfterSkipIntentsTxnBlock) {
  ...
  // Run a transaction block with CREATE TABLE + INSERT that uses skip-intents.
  auto writes_before = ASSERT_RESULT(GetSkipIntentsCount());

  ASSERT_OK(conn.Execute("BEGIN"));
  ASSERT_OK(conn.Execute("CREATE TABLE txn_pitr_test (id INT PRIMARY KEY)"));
  ASSERT_OK(conn.Execute(
      "INSERT INTO txn_pitr_test SELECT g FROM generate_series(1, 200) g"));
  ASSERT_OK(conn.Execute("COMMIT"));

  auto writes_after = ASSERT_RESULT(GetSkipIntentsCount());
  LOG(INFO) << "Skip-intents writes during txn block: "
            << writes_before << " -> " << writes_after;
  ASSERT_GT(writes_after, writes_before)
      << "Transaction block should have used skip-intents";

  // Verify data exists before restore.
  auto count = ASSERT_RESULT(
      conn.FetchRow<PGUint64>("SELECT count(*) FROM txn_pitr_test"));
  ASSERT_EQ(count, 200);

  // Restore to before the transaction.
  LOG(INFO) << "Restoring to " << restore_time;
  auto snapshot_id = ASSERT_RESULT(
      snapshot_util_.PickSuitableSnapshot(schedule_id, restore_time));
  ASSERT_OK(snapshot_util_.RestoreSnapshot(snapshot_id, restore_time));
  ...
  client::VerifyTableNotExists(client_.get(), kDbName, "txn_pitr_test", 30);

  conn = ASSERT_RESULT(Connect());
  auto result = conn.Execute("SELECT count(*) FROM txn_pitr_test");
  ASSERT_NOK(result) << "Table txn_pitr_test should not exist after PITR restore";
