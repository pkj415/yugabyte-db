...
DEFINE_RUNTIME_uint64(table_mutation_count_sender_interval_ms, 5 * 1000,
...
class TableMutationCountSender::Thread {
 ...
 private:
  ...
  std::string master_addresses_string_ GUARDED_BY(master_address_mtx_);
...
void TableMutationCountSender::Thread::RunThread() EXCLUDES(mutex_) {
  while (true) {
...
Status TableMutationCountSender::Thread::Start() EXCLUDES(mutex_) {
  RSTATUS_DCHECK(thread_ == nullptr, InternalError, "Table mutation sender thread does not exist");
...
Status TableMutationCountSender::Thread::Stop() EXCLUDES(mutex_) {
  if (!thread_) {
    return Status::OK();
  }
