...
class PgClientServiceImpl::Impl {
 public:
  ...
  Status Heartbeat(
    ...
    auto session_info = SessionInfo::Make(
        ...
        *GetYsqlAdvisoryLocksTable(), session_id, &client(), clock_, &table_cache_,
 ...
 private:
  ...
  YsqlAdvisoryLocksTable* GetYsqlAdvisoryLocksTable() {
    auto table = advisory_locks_table_.load(std::memory_order_acquire);
    if (!table) {
      std::lock_guard<std::mutex> lock(advisory_locks_table_mutex_);
      if (!advisory_locks_table_holder_) {
        advisory_locks_table_holder_ = std::make_unique<YsqlAdvisoryLocksTable>(client());
        advisory_locks_table_.store(
            advisory_locks_table_holder_.get(), std::memory_order_release);
      }
    }
    return advisory_locks_table_.load(std::memory_order_acquire);
  }
  ...
  std::mutex advisory_locks_table_mutex_;
  std::atomic<YsqlAdvisoryLocksTable*> advisory_locks_table_;
  std::unique_ptr<YsqlAdvisoryLocksTable> advisory_locks_table_holder_;
};
